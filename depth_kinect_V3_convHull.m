...
%         ptCloud = pcfromkinect(depthDevice, depthImage);
    ...
    roi = [-0.25, 0.18, -0.0375, 0.0275, 0, inf] %--> teste para limitar scanneamento (slice - com esteira) comprimento = 0.065,largura = 0,48(resultado bons)<-
...
function [height, width, depth,ptCloudB,number_of_Obj_samples] = pc_Object_Dimension_scanner(background_Distance,roi_Slice,depthDevice,colorDevice, method, step, cut_value,sample_rate)
    ...
    try
        ...
        disp(" ")
        volume_AABB = (Width_AABB*Hight_AABB*Depth_AABB);
        disp(Width_AABB + " X " + Hight_AABB + " X " + Depth_AABB + " | " + volume_AABB);
        
        disp(" ")
        disp("mensurement_OMBB:")
        disp(" ")
        disp(widthMBB + " X " +  heightMBB + " X " + depthMBB + " | " + volume_MBB);
        ...
        disp("O tempo de amostragem foi: " + (tempo_amostragem-6) + "s"); % a contagem de flag demora 6 segundos
        disp("O tempo de tratamento da ptCloud foi: " + tempo_tratamento_ptCloud + "s");
        disp("O tempo de medida com AABB foi: " + (tempo_medida_AABB + (tempo_amostragem-6) + tempo_tratamento_ptCloud) + "s");
        disp("O tempo de medida com OMBB foi: " + (tempo_medida_OMBB + (tempo_amostragem-6) + tempo_tratamento_ptCloud)+ "s");
        disp("O tempo de medida com OMBB 3D foi: " + (tempo_medida_OMBB3D + (tempo_amostragem-6) + tempo_tratamento_ptCloud) + "s");
         disp("A quantidade de amostras foi: " + number_of_Obj_samples);
        
        resultados = Results;
        
        resultados.Height = Hight_AABB;
        resultados.Width = Width_AABB;
        resultados.Depth = Depth_AABB;
        resultados.tempo_amostragem = tempo_amostragem;
        resultados.tempo_tratamento_ptCloud = tempo_tratamento_ptCloud;
        resultados.tempo_medida = tempo_medida_OMBB;
        resultados.quantidade_de_amostras = number_of_Obj_samples;

        
        %print_result_datas(resultados);
